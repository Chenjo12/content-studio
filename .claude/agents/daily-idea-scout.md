...
**Check for a same-day rerun before writing.** If `research/ideas/<date>-video-ideas.md` already exists (e.g. a manual rerun after the automated run already fired that day), don't overwrite it — write to `research/ideas/<date>-video-ideas-2.md` instead (increment further, `-3` etc., if that also exists). Note in the new file's header what prompted the rerun if you know.
